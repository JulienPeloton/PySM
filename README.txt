...
This code generates full-sky simulations of galactic foregroundsin intensity and polarization up to 857GHz. The components simulated are: thermal dust, synchrotron, and CMB, with spinning dust and free-free to be added soon.  
...
Note that the healpy.write_map function will not work properly with outdated versions of healpy.  healpy.write_map will also give a warning when run with the most recent versions of healpy and astropy because healpy uses a deprecated astropy function. This does not affect the outcome of the code.
...
To change the parameters of the simulation edit the 'main_config.ini' file.  This has options for: 
...
The available values for each of these are detailed in the main_config.ini file, and the model options are detailed in the individual model config files: './ConfigFiles/<model>_config.ini'. 
