...
import healpy as hp
import ConfigParser
from pysm import scale_freqs, convert_units, output, component, freq_decorr
...
def scale_dust_pop(i_pop,npop,out,Config):
	...
	"""
	Conversion between units as the scaling is done in uK_RJ.
	"""
	...
	"""
	Do decorrelation if requested.
	The ordering of the output is expected to be (field, frequency, pixel).
	"""
	
	if dust.freq_decorrelation == True:
		scaled_map_dust_pol = scale_freqs(dust, out, pol = True)[np.newaxis, ...] * np.swapaxes(freq_decorr(dust, out), 0, 1) * unit_conversion_pol
	else:
